  ...
  return (
    <section className="py-32 px-6 bg-white border-b border-black/10">
      <div className="max-w-[1440px] mx-auto">
        <SlideInLeft>
          <div className="mb-24 text-center md:text-left flex flex-col md:flex-row justify-between items-end gap-6 border-b border-black/10 pb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-black/10">
          {data.benefits.map((benefit, i) => (
            <div key={i} className="bg-white p-12 lg:p-20 group hover:bg-[#F4F1ED]/50 transition-colors duration-500">
              {i % 2 === 0 ? (
                <SlideInLeft delay={0.1}>
                  <div className="flex flex-col h-full justify-center">
                    <span className="text-black/30 font-poppins text-xs tracking-[0.2em] mb-4 block leading-none">0{i+1}</span>
                    <h3 className="font-poppins text-xl font-medium uppercase tracking-[0.1em] text-black mb-8">{benefit.title}</h3>
