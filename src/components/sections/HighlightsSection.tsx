...
const getIcon = (name: string) => {
  switch (name) {
    case "Leaf": return <Leaf className="w-6 h-6 text-black" strokeWidth={1} />;
    case "ShieldCheck": return <ShieldCheck className="w-6 h-6 text-black" strokeWidth={1} />;
    case "Users": return <Users className="w-6 h-6 text-black" strokeWidth={1} />;
    case "HandHeart": return <HandHeart className="w-6 h-6 text-black" strokeWidth={1} />;
    default: return <Leaf className="w-6 h-6 text-black" strokeWidth={1} />;
  }
};
        ...
        {data.highlights.map((item, i) => (
          <ZoomIn key={i} delay={i * 0.1}>
            <div className="flex flex-col items-center text-center group">
              ...
              <p className="text-black/60 leading-loose text-sm font-light max-w-xs">{item.desc}</p>
            </div>
          </ZoomIn>
        ))}
      </div>
